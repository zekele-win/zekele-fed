...
export const GET: RequestHandler = async ({ params, url, platform }) => {
  ...
  let lang = params.lang ? (params.lang.toLowerCase() as Lang) : defLang;
  if (!supportedLangs.includes(lang)) lang = defLang;
  const i18n = i18ns[lang];

  const brandSvg = brandSvgRaw.replace(/<svg[^>]*>/, "").replace("</svg>", "");
